...
export default function App() {
  ...
  const runAnalysis = async () => {
    ...
    try {
        ...
        // Fallback to Mock Data to keep the app functional
        const now = Math.floor(Date.now() / 1000);
        const mockCandles: Candle[] = [];
        let price = 1.0850;
        for(let i = 200; i > 0; i--) {
            const time = now - (i * 900); // 15m
            const open = price;
        ...
        const analysis = analyzeMarketHistory(mockCandles, mockCandles, mockCandles);
        setZones(analysis.zones);
        setMarkers(analysis.markers);
        setLiquidityLevels(analysis.liquidityLevels);
        setMarketBias(analysis.bias);
        
        setNotifications(prev => [{id: Math.random(), time: new Date().toLocaleTimeString(), title: "Veri Hatası", message: "Demo Verisi Aktif (CORS).", type: 'warning'}, ...prev]);
    }
  };
