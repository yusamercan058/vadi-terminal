...
import { Candle, SMCZone, ChartMarker, LiquidityLevel } from '../types';
...
const SmartChart: React.FC<SmartChartProps> = ({ data, zones, markers, liquidityLevels = [] }) => {
    const chartContainerRef = useRef<HTMLDivElement>(null);
    const chartRef = useRef<IChartApi | null>(null);
    const seriesRef = useRef<ISeriesApi<"Candlestick"> | any>(null);
    const histogramRef = useRef<ISeriesApi<"Histogram"> | any>(null);
    ...
    useEffect(() => {
        ...
        // 3. Clear Previous Lines
        activeLinesRef.current.forEach(line => {
             if (seriesRef.current?.removePriceLine) {
        ...
        activeZones.forEach(z => {
            if (!seriesRef.current?.createPriceLine) return;
